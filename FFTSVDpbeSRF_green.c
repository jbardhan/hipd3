...
int main(int argc, char* argv[]) {
  ...
  real energy = 0.0;
   ...
   setlinebuf(stdout);
   usequalocation = 0;  // thus using "Green thm" formulation of Yoon+Lenhoff
   ...
   problem = PBEproblem_allocate(argv[4], argv[5], 1);
	...
	SurfaceOperator_writematlabfile("GreenThmMat.m", problem->pbesurfaceoperator, problem->numtotalpanels);
...
/*    printf("reaction potentials = \n"); */
	...
	Vector3D* pointList = problem->pbesurfaceoperator->charges->points;
	Vector charges = problem->pbesurfaceoperator->charges->charges;
	if (argc > 5) {
	  for (i = 0; i < problem->numpdbentries; i++) {
		 //		 printf("i = %d, ce = %f\n", i, CoulombEnergy);
